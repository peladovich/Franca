...
<h1 class="font-headline-md text-headline-md text-primary mb-lg"><?= $item ? 'Edit' : 'Add' ?> Menu Item</h1>
...
<form method="post" enctype="multipart/form-data" class="bg-surface-container-lowest rounded-xl p-lg editorial-shadow space-y-4 max-w-2xl">
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    <div>
      <label class="font-label-md text-primary block mb-1">Name</label>
    ...
    <div>
      <label class="font-label-md text-primary block mb-1">Category</label>
  ...
  <div>
    <label class="font-label-md text-primary block mb-1">Description</label>
  ...
  <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
    <div>
      <label class="font-label-md text-primary block mb-1">Price</label>
    ...
    <div>
      <label class="font-label-md text-primary block mb-1">Badge (optional)</label>
    ...
    <div>
      <label class="font-label-md text-primary block mb-1">Sort order</label>
  ...
  <div>
    <label class="font-label-md text-primary block mb-1">Ingredients (comma-separated)</label>
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    <div>
      <label class="font-label-md text-primary block mb-1">Existing image</label>
    ...
    <div>
      <label class="font-label-md text-primary block mb-1">Or upload new image</label>
  ...
  <?php if (!empty($item['image'])): ?>
    <img src="<?= img_url($item['image']) ?>" class="w-24 h-24 object-cover rounded-lg" alt="Current image">
  <?php endif; ?>

  <div class="flex gap-6">
    <label class="flex items-center gap-2 font-label-md text-primary">
      <input type="checkbox" name="is_featured" <?= !empty($item['is_featured']) || (isset($_POST['is_featured'])) ? 'checked' : '' ?>> Featured on homepage
    </label>
    <label class="flex items-center gap-2 font-label-md text-primary">
      <input type="checkbox" name="is_available" <?= ($item === null || !empty($item['is_available'])) || isset($_POST['is_available']) ? 'checked' : '' ?>> Available
    </label>
  </div>

  <div class="flex flex-col sm:flex-row gap-3">
    <button class="w-full sm:w-auto text-center bg-primary text-on-primary px-lg py-3 rounded-lg font-label-md" type="submit"><?= $item ? 'Save Changes' : 'Create Item' ?></button>
    <a href="<?= BASE_URL ?>/admin/menu-items.php" class="w-full sm:w-auto text-center px-lg py-3 rounded-lg font-label-md text-on-surface-variant border border-outline-variant/50">Cancel</a>
  </div>
</form>
